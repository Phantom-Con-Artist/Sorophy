...
public static class PerformanceBenchmarkTests
{
        ...
        int graphSize)
    {
        ...
        /*
         * Relationship identities have single-lifetime semantics once
         * they are removed from the active graph. The benchmark therefore
         * uses a distinct deterministic identity set when rebuilding the
         * graph after the removal measurement.
         *
         * The transformation is bijective for the generated IDs, so the
         * replacement IDs are guaranteed to differ from the original IDs
         * while preserving deterministic benchmark behavior.
         */
        var replacementRelationshipIds =
            CreateReplacementRelationshipIds(
                relationshipIds);
        ...
        /*
         * Rebuild the graph with fresh relationship identities.
         *
         * The removed relationship identities are permanently retired by
         * SorophyGraph, so reusing them here would measure identity reuse rather
         * than relationship-storage performance.
         */
        for (var index = 0;
             ...
             index++)
        {
            graph.AddRelationship(
                ...
                {
                    Id =
                        replacementRelationshipIds[index],
    ...
    private static Guid[] CreateReplacementRelationshipIds(
        IReadOnlyList<Guid> relationshipIds)
    {
        var result =
            new Guid[relationshipIds.Count];

        for (var index = 0;
             index < relationshipIds.Count;
             index++)
        {
            var bytes =
                relationshipIds[index].ToByteArray();

            /*
             * XORing the same byte with a fixed non-zero mask is a
             * bijection over the original identity set. Therefore:
             *
             *   original ID != replacement ID
             *   distinct originals remain distinct replacements
             */
            bytes[0] ^=
                0xFF;
